...
def recursive_scan_double_hash(rootpath, known_folder_hashes=None, progress=None):
    ...
    for child in children:
        ...
        if os.path.isdir(full_path):
            child_doublons = recursive_scan_double_hash(full_path, known_folder_hashes, progress)
...
def export_csv(doublons: dict, output_file: str = "output.csv"):
    try:
        with open(output_file, mode="w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Doublon", "Original", "Taille (humain), Taille (Tri)"])
            for doublon, original in doublons.items():
                try:
                    ...
                    size = "?"
                writer.writerow([doublon, original, print_size(size), size])
        print(f"Fichier CSV généré : {output_file}")
    except Exception as e:
        print(f"Erreur lors de la génération du fichier CSV : {e}")
...
def scan_double_hash(rootpath):
    ...
    # Compte les dossiers non cachés pour la progression
    total_to_scan = sum(
        len([d for d in dirs if not is_hidden(d)])
        for _, dirs, _ in os.walk(rootpath)
    )

    with tqdm(total=total_to_scan, desc="Analyse des dossiers") as progress:
        doublons = recursive_scan_double_hash(rootpath, progress=progress)
